...
# 507 "parson.c"
                           ;
    unsigned int i = 0;
    size_t new_capacity = ((object->cell_capacity * 2) > (16) ? (object->cell_capacity * 2) : (16));
...
static JSON_Status json_array_add(JSON_Array *array, JSON_Value *value) {
    if (array->count >= array->capacity) {
        size_t new_capacity = ((array->capacity * 2) > (16) ? (array->capacity * 2) : (16));
...
# 1141 "parson.c"
            ...
            for (i = 0; i < count; i++) {
                if (is_pretty) {
                    do { int level_i = 0; for (level_i = 0; level_i < (level+1); level_i++) { do { written = (sizeof(("    ")) - 1); if (buf != 
# 1145 "parson.c" 3 4
                   ((void *)0)
# 1145 "parson.c"
